...
if (isset($_GET['hapus'])) {
    ...
    $ambil = $koneksi->query("SELECT gambar FROM produk WHERE id='$id_hapus'");
    $pecah = $ambil->fetch_assoc();
    $foto  = $pecah['gambar'];
    if (file_exists("assets/gambar/$foto")) {
...
<head>
    ...
    <style>
        body { background: #cbb79c; font-family: 'Poppins', sans-serif; }
        .navbar-custom { background: #561c24; }
        .navbar-brand { display: flex; align-items: center; gap: 10px; }
        .navbar-brand img { height: 40px; border-radius: 4px; }
        .card { background: #e8d8c4; border-radius: 12px; }
        h2 { color: #561c24; font-weight: 600; }
        .produk-card { background: #f1e4d3; border-radius: 14px; transition: 0.3s; }
        .produk-card:hover { background: #dcc8ad; transform: translateY(-5px); }
        .produk-img { width: 100%; height: 160px; object-fit: cover; border-radius: 10px; }
        .produk-card h6 { color: #561c24; }
        
        /* Tombol Tambah Custom */
        .btn-add { background: #561c24; color: white; border-radius: 10px; font-weight: 600; padding: 10px 20px; transition: 0.3s; border: none; }
        .btn-add:hover { background: #6d2932; color: #f1e4d3; transform: scale(1.05); }

        .icon-btn { width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; border-radius: 50%; background: #e8d8c4; text-decoration: none; transition: 0.2s; }
        .icon-btn i { font-size: 18px; color: #561c24; }
        .icon-btn:hover { background: #dcc8ad; transform: scale(1.1); }
    </style>
</head>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto">
                ...
                <!-- <li class="nav-item"><a class="nav-link" href="admin_tambah_produk.php"><i class="bi bi-plus-circle me-1"></i> Tambah Produk</a></li> -->
        ...
        <div class="row">
            ...
            <div class="col-md-4 col-sm-6 mb-4">
                <div class="produk-card p-3 h-100 text-center">
                    ...
                    <p class="mb-1">Rp <?php echo number_format($row['harga'], 0, ',', '.'); ?>/kg</p>
                    
                    <?php if ($row['stok'] > 0) { ?>
                        <span class="badge bg-success mb-2"><?php echo $row['stok']; ?> Item</span>
                    <?php } else { ?>
                        <span class="badge bg-danger mb-2">Habis</span>
                    <?php } ?>

                    <div class="d-flex justify-content-center gap-2 mt-2">
                        <a href="edit_produk.php?id=<?php echo $row['id']; ?>" class="btn icon-btn edit-btn">
